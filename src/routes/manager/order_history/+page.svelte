<script>
  ...
  onMount(() => {
    get_statuses();
    get_orders_total(from_date, to_date, checked_statuses);
    get_customer_orders(from_date, to_date, checked_statuses, offset, limit);
    return () => {}; 
  });
  ...
  async function get_orders_total(from_date, to_date, checked_statuses) {
    const route_input = `/manager/order_history/get_orders_total`;
  ...
  async function get_customer_orders(from_date, to_date, checked_statuses, offset, limit) {
  ...
  async function update_order(customer_order_id, status_id) {
    const output = await patch_ingredients(customer_order_id, status_id);
    if (output) 
