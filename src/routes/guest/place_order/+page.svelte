<script>
  ...
  $: id = $auth.id;
  $: if (!import.meta.env.SSR &&  category !== 'guest')
  ...
  async function get_menuitems() {
        let input = `/guest/place_order/get_menu_items`;
  ...
  onMount(() => { 
    ...
    async function insert_ordered_items(id, items) {
     ...
     const options = {
      ...
      const response = await fetch('/guest/place_order/post_items', options);
    }
    ...
    async function insert_order() {
      ...
      const response = await fetch('/guest/place_order/post', options);
              ...
              <tbody>
                {#each menuitems as menuitem }
                  <tr>
                    ...
                    <td> 
                      <Image src = {get_image(menuitem.menu_item_id)} alt = {menuitem.name} width="96" height="96"/>
                    </td>
                    <td>{menuitem.name}</td>
